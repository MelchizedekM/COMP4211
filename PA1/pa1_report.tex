...
\subsection*{Q1.}
\begin{itemize}
    \item The training set contains 3539 samples(instances), each with 31 features (exclude two labels, `regression targets' and `classification targets') and 2 labels (`regression labels' and `classification labels').
    \item Categorical features: C0, C1, C2, C3, C4, C5, C7, C8, C9, C10, C11, C12, C13, C15.
    \item Numerical Features: C6, C14, C16, C17, C18, C19, C20, C21, C22, C23, C24, C25, C26,
    C27, C28, C29, C30.
\end{itemize}
...
Binary features: C4, C8, C9, C10, C11, C12, C13, C15.\\
Nominal features: C0, C1, C3, C7.\\ 
Ordinal features: C2, C5.\\
...
If we can reduce the dimensionality of the data using some feature engineering, we may not only reduce the complexity of the model but also avoid the multicollinearity. Also it can reduce the possibility of overfitting.
...
Moreover, since there will be more information in six principal components than in one principal component, the $R^2$ and MSE of the model using six principal components are higher than the $R^2$ and MSE of the model using only one principal component.
...
After fitting a linear regression model using a categorical feature (C1), we find that:
...
However, the ROC gives more information than AUC, such as the trade-off between the true positive rate and the false positive rate. Therefore, we may also consider to use the ROC curve to evaluate the performance of the model.

Normally, the most top-left point of the ROC curve is the best point, which means a relatively higher true positive rate and a relatively lower false positive rate.
    
\subsection*{Q19.}

From the table \ref{tab:model_performance_constant_lr}, as the learning rate increases, the training time decreases, the F1 score and the testing accuracy decrease, and the training accuracy decreases.
...
\begin{table}[htbp]
    ...
    \midrule
    \multirow{3}{*}{1} & 0.0900 & 0.3037 & 0.1415 & \multirow{3}{*}{0.0919 $\pm$ 0.0013} & \multirow{3}{*}{0.4346 $\pm$ 0.1851} & \multirow{3}{*}{0.2849 $\pm$ 0.2028} \\
                       & 0.0929 & 0.3037 & 0.1415 & & \\
                       & 0.0927 & 0.6963 & 0.5717 & & \\
    \midrule
    \multirow{3}{*}{8} & 0.3188 & 0.8588 & 0.8520 & \multirow{3}{*}{0.1970 $\pm$ 0.0869} & \multirow{3}{*}{0.8395 $\pm$ 0.0156} & \multirow{3}{*}{0.8312 $\pm$ 0.0178} \\
                       & 0.1505 & 0.8390 & 0.8332 & & \\
                       & 0.1216 & 0.8206 & 0.8085 & & \\
    \midrule
    \multirow{3}{*}{32} & 0.3623 & 0.8630 & 0.8571 & \multirow{3}{*}{0.3744 $\pm$ 0.0140} & \multirow{3}{*}{0.8611 $\pm$ 0.0070} & \multirow{3}{*}{0.8551 $\pm$ 0.0085} \\
                        & 0.3667 & 0.8517 & 0.8438 & & \\
                        & 0.3941 & 0.8686 & 0.8643 & & \\
    \midrule
    \multirow{3}{*}{128} & 0.7841 & 0.8602 & 0.8553 & \multirow{3}{*}{0.7050 $\pm$ 0.0748} & \multirow{3}{*}{0.8616 $\pm$ 0.0031} & \multirow{3}{*}{0.8567 $\pm$ 0.0014} \\
                         & 0.6046 & 0.8588 & 0.8563 & & \\
                         & 0.7261 & 0.8658 & 0.8586 & & \\
    \bottomrule
    \end{tabular}
...
However, the training time of the best model is 0.7050 $\pm$ 0.0748, which is relatively high comparing to the training time of the Logistic Regression model (which is no higher than 0.01s).
...
Moreover, the spead of increase of the accuracy and F1 score is decreasing as the hidden units increases. The possible reason could be that the model has already good enough to fit the data, so the increase of the hidden units could not make the model to be better.
\newpage
